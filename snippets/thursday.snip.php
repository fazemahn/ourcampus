<h4>Here's what Thursday looks like:</h4>
<script>
document.getElementById("mon").classList.remove('active');
document.getElementById("thu").classList.add('active');
</script>
<?php
require 'includes/dbhandler.inc.php';
$sql = "SELECT * FROM ".$_SESSION['UserName']." WHERE thu IS NOT NULL;";
$result = mysqli_query($conn, $sql);
//store all courses by id into $Courses array
if (mysqli_num_rows($result) > 0) {
    $Courses=array();
    while($row = mysqli_fetch_assoc($result)) {
      array_push($Courses,$row["thu"]);
      //print_r($Courses);
    }
} else {
